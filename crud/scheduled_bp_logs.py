...
def get_logs_by_schedule_id(db: Session, schedule_id: int) -> List[ScheduledBPLog]:
...
def get_logs_by_date_range(db: Session, user_id: int, start_date: date, end_date: date) -> List[ScheduledBPLog]:
...
def get_logs_by_date(db: Session, user_id: int, target_date: date) -> List[ScheduledBPLog]:
